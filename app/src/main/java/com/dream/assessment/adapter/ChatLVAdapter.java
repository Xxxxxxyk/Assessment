...
import com.dream.assessment.gif.AnimatedGifDrawable;
import com.dream.assessment.gif.AnimatedImageSpan;
...
public class ChatLVAdapter extends BaseAdapter {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			hodler.chatto_content_img = (ImageView) convertView.findViewById(R.id.chatto_content_img);
		...
		if (list.get(position).fromOrTo == 0) {
			...
			hodler.chatto_content_img.setVisibility(View.GONE);
		...
		} else if(list.get(position).fromOrTo == 1){
			// 发送消息 to显示
			hodler.chatto_content_img.setVisibility(View.GONE);
			...
			hodler.chatto_content_img.setVisibility(View.VISIBLE);
			hodler.toContainer.setVisibility(View.VISIBLE);
			hodler.fromContainer.setVisibility(View.GONE);
            hodler.toContent.setVisibility(View.GONE);

			Bitmap bitmap = BitmapFactory.decodeFile(list.get(position).content);
			//加载图片
			Glide.with(mContext).load(list.get(position).content).into(hodler.chatto_content_img);
			hodler.time.setText(list.get(position).time);
		}
	...
	class ViewHodler {
		ImageView fromIcon, toIcon ,chatto_content_img;
